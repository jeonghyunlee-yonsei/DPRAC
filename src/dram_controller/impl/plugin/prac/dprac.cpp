...
class DPRAC : public IControllerPlugin, public Implementation, public IPRAC {
    RAMULATOR_REGISTER_IMPLEMENTATION(IControllerPlugin, DPRAC, "DPRAC", "DPRAC.")
...
private:
    DeviceConfig m_cfg;
    std::vector<DPRAC::PerBankCounters> m_bank_counters;
...
void handle_refresh_request(const Request& req) {
    ...
    for (auto& counter : m_bank_counters) {
        ...
        auto& recently_accessed_rows = counter.get_recently_accessed_rows();
        auto& list1 = counter.get_list1(); // 각 뱅크의 list1에 접근
        auto& list2 = counter.get_list2(); // 각 뱅크의 list2에 접근
    ...
    // m_thres_rows에서 해당 row 제거
              ...
              if (act_max != counters.end()) {
                 counters[act_max->first] = 0;
                        std::printf("[Before] [Bank %d] Removed first Row: %d from m_thres_rows with Count: %u and reset its COUNT in m_counters.\n",
                             counter.get_bank_id(), act_max->first, max_count);
    ...
    // 두 번째로 큰 값을 가진 row를 찾음
            ...
            // m_counters에서 해당 row의 count 초기화
              auto second_act_max = counters.find(second_row_addr);
                    ...
                    counters[second_act_max->first] = 0;
                       ...
                       std::printf("[Before] [Bank %d] Removed second Row: %d from m_thres_rows with Count: %u and reset its COUNT in m_counters.\n",
        ...
        if (trefi_reset) {
            // 현재 클럭이 8000 단위로 나누어 떨어질 때만 실행
            if (m_clk % 8000 == 0) {
                // 2번 구간이 끝나고 3번 구간이 시작될 때
                if (m_clk % 16000 == 0) {
                    // list2를 비우고, list1을 list2로 이동
                    list2.clear();
                    list2 = std::move(list1);
                    list1.clear();
                    if (m_debug) {
                        std::printf("[PRAC] [%ld] Transition to next phase: list1 -> list2, list1 cleared.\n", m_clk);
                    }
                }

                // 현재 구간 동안 접근이 없는 항목 초기화
                for (auto it = counters.begin(); it != counters.end(); ++it) {
                    if (it->second != 0 && list2.find(it->first) == list2.end()) {
                        if (m_debug) {
                            std::printf("[PRAC] [%d] Resetting Count for Row: %d. Previous Count: %u\n", 
                                        counter.get_bank_id(), it->first, it->second);
                        }
                ...
                if (m_debug) {
                    std::printf("[PRAC] [%ld] Completed reset for rows not in list2.\n", m_clk);
                }
            }
        }
    }
...
class PerBankCounters {
    public: 
    const std::unordered_set<int>& get_list1() const {
        return list1;
    }

    std::unordered_set<int>& get_list1() {
        return list1;
    }

    const std::unordered_set<int>& get_list2() const {
        return list2;
    }

    std::unordered_set<int>& get_list2() {
        return list2;
    }
    ...
    std::unordered_set<int> list1; // 현재 구간 동안 ACT된 row를 저장
    std::unordered_set<int> list2; // 이전 구간 동안 ACT된 row를 저장
